...
выбранного макета на `src/source-snapshots/*.html`. Поле `render: "native"` в JSON выключает
снимок для конкретного маршрута; JSON всё равно задаёт SEO и Schema.org. Снимки, манифест и
файлы редиректов руками не правят — только их источники и генераторы.
...
| `docs/DIZAYN-PRODUKTOVYH-STRANIC.md` | Токены, иконки, анимации и компоненты продуктового дизайна | Сборка новой нативной страницы и развитие общей системы |
| `src/components/product/`, `src/layouts/ProductQuestPage.astro` | Нативные продуктовые блоки квестов и праздников; включаются данными страницы | Перевод страницы со снимка, порядок блоков, CTA, FAQ, галерея, карта или бронь |
| `src/data/video-hls.json`, `src/components/HlsVideo.astro`, `scripts/video/` | Реестр HLS-видео, ленивый плеер и конвейер публикации | Добавление подтверждённого трейлера или ролика праздника |
...
### Перевести квест или праздник на продуктовую страницу

1. Подтверди все факты, медиа, отзывы и карту; блок без данных не выводи. Полная схема, порядок
   блоков и проверки описаны в `docs/PRODUKTOVAYA-STRANICA.md`.
2. Поставь `render: "native"` в JSON страницы. Это выключит её Tilda-снимок без ветвления по slug;
   квест использует `product`, а праздник — заполненные `sections[]`.
3. Для ролика сначала добавь подтверждённую запись в `src/data/video-hls.json` и прогони конвейер
   из `docs/VIDEO-HLS.md`; в данные страницы передавай его slug, а не URL видеофайла.
4. Проверь нативное тело без `source-snapshot-shell`, CTA и якоря, FAQ, карту, видео и снимки на
   390, 768 и 1440 px. Затем запусти релевантные тесты, SEO-аудит и полную сборку.
