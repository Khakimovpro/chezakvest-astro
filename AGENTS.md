...
В `sitemap.xml` 64 индексируемые страницы; контент 63 внутренних страниц лежит в JSON.
...
| --- | --- | --- |
| `src/data/pages/<slug>.json` | Данные страницы и её SEO; 63 файла, типы ниже | Контент, новая страница, связь квеста с площадкой |
...
| `src/pages/kvesty-v-rostove-na-donu.astro` | Нативный автокаталог всех `type: quest` | Фильтры и карточка каталога |
...
5. Каталог подхватит карточку автоматически, но вручную обнови числовой текст `catalog из 40 игр`
...
7. Тест `tests/seo-data.test.mjs` сейчас точно ожидает 64 индексируемые страницы. Постоянное
