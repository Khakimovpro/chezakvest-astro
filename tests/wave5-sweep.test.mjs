...
test('uses the owner-approved local lead endpoint and only confirmed analytics', async () => {
  ...
  assert.equal(site.leads.recipient, '/api/lead');
  assert.equal(site.analytics.metrikaId, '48864086');
  assert.match(leadForm, /if \(!endpoint\) throw new Error/);
