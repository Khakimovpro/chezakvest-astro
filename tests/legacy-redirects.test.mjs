...
test('legacy URL map contains every audited URL and the Wednesday consolidation', async () => {
  ...
  assert.deepEqual(
    ...
    ['/new-year-2025', '/wednesday_ukradennaya_vesch'],
  );
  ...
  assert.deepEqual(
    ...
    [
      ...
      '/prazdniki-pod-kluch', '/privacy', '/roblox-land',
    ],
  );
});
...
test('generator emits exact Apache and nginx redirects without 200 self-routes', async () => {
  ...
  assert.match(htaccess, /^# Generated by `node migration\/build_legacy_redirects\.mjs`\. Do not edit\.$/m);
  assert.match(htaccess, /^RewriteRule \^page57307963\\\.html\$ \/minecraft\/ \[R=301,L\]$/m);
...
test('checked-in redirect artifacts are deterministic renderings of the source map', async () => {
  const entries = await loadLegacyUrlMap();
  const [redirects, htaccess, nginx, cutover] = await Promise.all([
    readFile(new URL('../public/_redirects', import.meta.url), 'utf8'),
    readFile(new URL('../.htaccess', import.meta.url), 'utf8'),
    readFile(new URL('../docs/nginx-legacy-redirects.conf', import.meta.url), 'utf8'),
    readFile(new URL('../docs/PRODUCTION_CUTOVER.md', import.meta.url), 'utf8'),
  ]);

  assert.equal(redirects, renderRedirects(entries));
  assert.equal(htaccess, renderHtaccess(entries));
  assert.equal(nginx, renderNginxRedirects(entries));
