...
**Перенесены все 58 страниц официального sitemap**: главная, 37 квестов, 4 VR-игры,
9 площадок, категория «Страшные квесты», «Контакты» и 5 праздничных страниц.
Дальше — волна 5: новые страницы под спрос, которых на Tilda нет (см. план волн).
...
Праздничные страницы ссылаются на лендинги, которых нет в официальном sitemap, но которые
живы на Tilda и отвечают 200: `/igra-v-kalmara-lend`, `/minecraft-lend`, `/roblox-land`,
`/amongus-land`, поддомен `гарри-поттер-и-кубок-огня.чезаквест.рф`, а также `/privacy` и
`/new-year`. Сейчас ссылки ведут на Tilda и в 404 не падают. При полном переезде домена
их нужно перенести — особенно `/privacy`: на неё ссылается каждая страница сайта.
...
| 5 | новые страницы под спрос из аудита (корпоратив, каникулы, тематические лендинги праздников) | их на Tilda нет — это рост, а не перенос | следующая |
