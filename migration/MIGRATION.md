...
- Шифр: AES-256-GCM, ключ из пароля через PBKDF2-HMAC-SHA256, 250 000 итераций,
  у каждого файла своя соль и свой nonce. Пароль нигде не хранится.
...
```bash
SITE_BASE=/chezakvest-preview npm run build
python3 _capture/encrypt_site.py "$(cat .preview-password)" --src dist --out dist-enc --base /chezakvest-preview/
# затем содержимое dist-enc запушить в main репозитория chezakvest-preview
```
