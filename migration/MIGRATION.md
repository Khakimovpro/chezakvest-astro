...
Обновлено: **16.08.2026**. Машиночитаемый реестр страниц — `migration/pages.csv`
(пересобирается `python3 migration/build_registry.py`).

## Статус одной строкой

**Финальный честный статус (16.08.2026):** R89 охватил все 67 Astro-маршрутов и 268 кадров на 1440×900/390×844 (DPR 2, touch): 64 `pass`, 2 `redirect_ok`, 1 ожидаемый `extra_clone`, **0 `needs_fix`**. Точечный повтор `/roblox-land/` в том же R89 checkpoint дал 95,04 % desktop и 88,59 % mobile (порог 88 %), 18/18 секций и нулевые health defects. Два независимых финальных width-круга содержат по 804 измерения (67×12) и `defectCount=0`; first-frame browser-audit проверил 66 source snapshots на 390px без blank/overflow/error состояния. Статический no-JS fallback local forms не отправляет PII, а route-scoped freeze home NLM216 firefly убрал зафиксированный CLS. Финальные CI/production/SEO и mobile Lighthouse evidence сохранены в `migration/parity/`. Итоговый статус и ссылки на публикации — [`parity/FINAL-STATUS-2026-08-15.md`](parity/FINAL-STATUS-2026-08-15.md).
...
## Архив: аудит parity — 11.08.2026 (исторический измеренный статус R15)
...
по клику. Исходники из снимков Tilda, которые не входят в опубликованный опыт, сохранены в
`migration/parity/source-media/`; публичная сборка не раздаёт их и не делает внешний запрос к
старому хосту.
